...
interface WorkspaceHeaderProps {
  ...
  /** Callback to import a workspace from a zip file */
    ...
    <div className='min-w-0 flex-1'>
      {isMounted && isCollapsed ? (
        <button
          ...
          className='group mx-0.5 flex h-[30px] cursor-pointer items-center rounded-lg px-2 transition-colors hover-hover:bg-[var(--surface-active)]'
        >
              ...
              <>
                {activeWorkspaceFull.logoUrl ? (
                  <img
                    ...
                    className='h-[16px] w-[16px] rounded-sm object-cover group-hover:invisible'
                  />
                ) : (
                  <div
                    className='flex h-[16px] w-[16px] items-center justify-center rounded-sm font-medium text-[9px] text-white leading-none group-hover:invisible'
                    style={{
                ...
                <PanelLeft
                  aria-hidden
                  className='pointer-events-none invisible absolute inset-0 m-auto h-[16px] w-[16px] rotate-180 text-[var(--text-icon)] group-hover:visible'
                />
              </>
            )}
          </div>
        ...
        >
          <DropdownMenuTrigger asChild>
            <button
              ...
              className={cn(
                'group mx-0.5 inline-flex h-[30px] min-w-0 max-w-full items-center rounded-lg px-2 transition-colors hover-hover:bg-[var(--surface-active)]',
              ...
              {activeWorkspaceFull ? (
                activeWorkspaceFull.logoUrl ? (
                  <img
                    src={activeWorkspaceFull.logoUrl}
                    alt={activeWorkspaceFull.name || 'Workspace logo'}
                    className='h-[16px] w-[16px] flex-shrink-0 rounded-sm object-cover'
                  />
                ) : (
                  <div
                    className='flex h-[16px] w-[16px] flex-shrink-0 items-center justify-center rounded-sm font-medium text-[9px] text-white leading-none'
                    style={{
                      backgroundColor: activeWorkspaceFull.color ?? 'var(--brand-accent)',
                    }}
                  >
          ...
          >
            {isWorkspacesLoading ? (
              ...
              <>
                <div className='-mx-1.5 -mt-1.5 flex max-h-[106px] flex-col gap-0.5 overflow-y-auto px-1.5 py-1.5'>
                    ...
                    return (
                      <div key={workspace.id}>
                        {editingWorkspaceId === workspace.id ? (
                          <div className='flex h-[30px] items-center gap-2 rounded-lg bg-[var(--surface-active)] px-2'>
                            {workspace.logoUrl ? (
                              <img
                          ...
                          <div
                            className={cn(
                              'group flex h-[30px] cursor-pointer select-none items-center gap-2 rounded-lg px-2 transition-colors',
                              !isActive && !isMenuOpen && 'hover-hover:bg-[var(--surface-active)]',
                              (isActive || isMenuOpen) && 'bg-[var(--surface-active)]'
                            )}
                            onClick={(e) => {
                ...
                <div className='flex flex-col gap-0.5'>
                  ...
                  >
                    <ArrowDown className='h-[14px] w-[14px] shrink-0 text-[var(--text-icon)]' />
                    Import workspace
                  </button>
                </div>
